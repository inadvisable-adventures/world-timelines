...
- avoid putting HTML and CSS directly in code. prefer `<template>` and `<slot>` where possible in web component code especially.
- all ingested data needs a citation to its origin site, including a direct link to the source item(s) where possible, and failing that, a generic link to the data source; maintain a list of licenses for each data source used.
