...
10. Improve the QLever `event` category query: (a) switch from exact to transitive Wikidata type matching, since the exact match returns ~0 events before 1200 CE (see `investigations/wikidata-event-transitive-type-match.md`); (b) exclude "sports season" items generally, the same pattern as sports-figure exclusion for `person` (see `investigations/wikidata-sports-exclusion.md`); (c) exclude a curated list of narrow/administrative election subtypes while keeping national/significant ones (see `investigations/wikidata-election-exclusion.md`). [planned: qlever-improve-event-category.md]
