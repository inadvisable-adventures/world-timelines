...
9. Bulk-download the full records (no binary blobs) of all ~1.24M `person` entries matching the current QLever query filters (TODO items 6–8), and store them in the local Postgres database as a JSONB document collection. Must be done without risking rate-limiting the shared public QLever endpoint. [planned: wikidata-bulk-person-download.md]
