...
| **OpenStreetMap** (plain OSM, `osm.org`/`api.openstreetmap.org` — Great Wall of China geometry, `db/fetch-great-wall.mjs` → `web-client/public/data/great-wall.json`) | **ODbL 1.0** | TODO item 14 (`plans/import-great-wall-boundary.md`). **Share-alike**: redistributing/adapting this data (the merged-and-length-capped polylines are a real transformation of the source) carries ODbL's copyleft obligations. Distinct from **OpenHistoricalMap** below, a related but separately-licensed (CC0) project this app does not currently ingest — do not conflate the two. |
