...
A second non-filter directive, `pin: id1, id2, ...` (TODO #16), lists entries to keep visible on the map/timeline regardless of what the query above matches — pinning adds entries back in rather than restricting the result set, so it's parsed alongside `laneset`/`limit`, not as a `DslFilter`. The results-bar "Pin all"/"Unpin all" buttons and each entry-detail panel's pin toggle are visual shortcuts that generate/update this line, the same bidirectional pattern as `category`/`laneset`. Pinned ids are resolved against the IndexedDB cache only (no live by-id fetch exists for the Wikidata/QLever source), with a Postgres fallback fetch for UUID-shaped ids missing from cache — see `plans/pin-unpin-and-dsl-line-folding.md`.
...
test batch); `entry-detail.ts`'s secondary Wikipedia cross-reference link
(TODO #15) is built from `wikipediaTitle`, not `title`, for exactly this
reason.
...
- A styled `<textarea>`, shown only while focused (the live edit surface —
  full raw text, no folding, since you need to see what you're editing).
- While blurred (at rest), a sibling rendered view shows one row per DSL
  line, truncated via CSS ellipsis when too long to fit — e.g. a `pin:`
  line listing several ids. Clicking a truncated row expands it in place
  (with a `[fold]` control, or re-clicking the text, to collapse it back);
  clicking a short row or empty space re-enters edit mode. True per-
  character folding inside a live `<textarea>` isn't possible, hence the
  separate at-rest view — see `plans/pin-unpin-and-dsl-line-folding.md`
  (TODO #16).
